...
const themes = [
  {
    ...
    seed: lightSeed,
    label: primaryButtonLabel.light,
  },
...
describe.each(themes)('$name theme contrast', ({ algorithm, seed, label }) => {
  ...
  it.each([
    ...
    [
      'description text on page background',
    ...
    [
      'primary as an interface component',
      () => t.colorPrimary,
      () => t.colorBgContainer,
      INTERFACE,
    ],
...
describe('palette rules', () => {
  ...
  it('the naive palette really does fail, so this gate is testing something', () => {
    const naive = theme.getDesignToken({
      algorithm: theme.darkAlgorithm,
      token: { colorPrimary: '#1f3a5f' },
    })
