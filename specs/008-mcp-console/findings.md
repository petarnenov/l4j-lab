...
**CLOSED by feature 010** (2026-09-15). Measured mechanically for the first time by
`ToolDeclarationContractTest`, which feature 007's build file always claimed existed and which was
never written: **106 differences**, not 18 — 71 in output schemas, 23 in input schemas, 10 in
annotations, 2 in descriptions. The keywords were not lost in transit; they were never there, because
the shapes were declared twice by hand. The Java is now the single source, `ToolArgumentConstraints`
carries what `@ToolArg` and the type system cannot, `McpRequestGate` enforces every keyword that is
declared, and the committed contracts are generated by `./gradlew :mcp-server:generateToolContracts`.
See 010 research R-002 and its addendum for why the constraints did not end up on the annotation.
...
**CLOSED by feature 010** (2026-09-15). `previous_fee_bps` is on the result, derived as
`new_fee_bps - delta_bps` — exact, and without a second call to a system that might have moved. The
replay path answers with the same field, including for operations recorded before it existed.
...
**CLOSED by feature 010** (2026-09-15). `serverInfo` carries `instance`, read from
`MCP_INSTANCE_ID` and set per replica in `compose.mcp.yaml`. Deliberately a property of the server
rather than an nginx header: the proxy is one deployment shape among several (010 research R-006).
The console reads it from the answer, and still declines to invent one when the answer names none.
...
**CLOSED by feature 010** (2026-09-15). The server was right and the contract was wrong
(010 research R-005): the call returns a handle, and the completed run arrives later through
`tasks/get`. Generating the contract from the declaration settled it without an argument.
...
**CLOSED by feature 010** (2026-09-15). `contracts/mcp-protocol.md` now writes the retry
out in full, including the `ElicitResult` envelope, with an example. The server tells three answers
apart where it told two: confirmed, declined, and **present but unreadable** — the last answered with
`isError` and a sentence naming `content.confirmed`, rather than being taken for a refusal. A decline
is a result with no `isError`, which is what the contract always said it was.
`ConfirmationTopologyTest` builds the retry from the corrected contract alone and applies the
change.
...
**CLOSED by feature 010** (2026-09-15), and **the finding was wrong about the cause**.
It has nothing to do with entitlement: *every* search whose result set was empty answered HTTP 500.
The legacy API omitted an empty `items`, the MCP server dereferenced the absent field, and the NPE
became an error with no message for the SDK to reject. Fixed at all three levels. A cross-advisor
search is an ordinary empty result — the system of record applies the entitlement predicate before it
counts — and is byte-for-byte identical to one for an advisor that does not exist, which is the
